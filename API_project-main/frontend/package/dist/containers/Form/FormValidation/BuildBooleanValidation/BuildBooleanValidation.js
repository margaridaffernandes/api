...
const buildBooleanValidation = (item, pathLabel, cardinalityObject, validationSchema, itemSection, sectionValidation, multipleSections, ruleMandatory) => {
  const checkMandatory = handleIfMandatoryValidation(item);

  if (ruleMandatory) {
    validationSchema[pathLabel] = Yup.string().required('Campo obrigatório');
  } else if (item.occurrences.lowerOccurrences === 1 && item.occurrences.upperOccurrences === 1) {
    ...
    validationSchema[pathLabel] = Yup.string().test("at-least-one", function (value) {
      const isVisible = handleRules(item, this.parent, "visibility");
