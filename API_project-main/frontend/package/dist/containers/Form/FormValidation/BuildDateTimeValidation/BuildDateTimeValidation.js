...
const buildDateTimeValidation = (item, pathLabel, cardinalityObject, validationSchema, itemSection, sectionValidation, multipleSections) => {
  ...
  if (item.occurrences.lowerOccurrences === 1 && item.occurrences.upperOccurrences === 1) {
    ...
    if (pathLabelArray[pathLabelArray.length - 1] === "date") {
      pathArray.push("time");
      let pathLabelTime = pathArray.join("-");
      validationSchema[pathLabel] = Yup.string().test(pathLabel, "Campo obrigatório", function (value) {
        ...
        let field = this.parent[pathLabelTime];
      ...
      pathArray.push("date");
      let pathLabelDate = pathArray.join("-");
      validationSchema[pathLabel] = Yup.string().test(pathLabel, "Campo obrigatório", function (value) {
        const isVisible = handleRules(item, this.parent, "visibility");
        ...
        let field = this.parent[pathLabelDate];
...
export { buildDateTimeValidation };